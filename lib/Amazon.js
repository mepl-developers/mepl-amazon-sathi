...
class AmazonScraper {
    ...
    grabReviews(body) {
        ...
        const reviewsList = $('#cm-cr-dp-review-list')[0].children;
        let scrapingResult = {};
        ...
        for (let key in scrapingResult) {
            ...
            try {
                scrapingResult[key].title = $(search[0]).text().split('out of 5 stars')[1].trim();
            } catch (error) {
                continue;
            }
        }
         ...
         */
        for (let key in scrapingResult) {
            const search = $(`#${key} [data-hook="review-collapsed"]`);
