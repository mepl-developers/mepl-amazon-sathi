...
class AmazonScraper {
    ...
    async startScraper() {
        ...
        return {
            ...
            ...(this.scrapeType === 'reviews_by_asin' ? { ...this.reviewMetadata } : {}),
        ...
        return new Promise((resolve, reject) => {
            forEachLimit(
                ...
                async (item) => {
                    ...
                    if (this.scrapeType === 'reviews_by_asin') {
                        this.grabReviews(body);
                    }
                    if (!this.bulk) {
                        throw new Error('Done');
    ...
    grabReviews(body) {
         ...
         */
        if (this.scrapeType == 'reviews') {
            const reviewListStat = $('#histogramTable')[0];
            try {
                reviewListStat.children.forEach((item) => {
                    const star = parseInt($(item).find('.a-section.a-spacing-none.a-text-left').text().match(/\d+/g)[5]);
                    const percentage = $(item).find('.a-section.a-spacing-none.a-text-right').text().match(/\d+/g)[5] + '%';
        ...
        if (this.scrapeType == 'reviews_by_asin') {
            const reviewListItems = $('#histogramTable').find('li');

            try {
                reviewListItems.each((index, element) => {
                    const $el = $(element);

                    const starText = $el.find('.a-text-left').first().text().trim().split(' ');
                    const starMatch = starText[index].match(/(\d+)/);
                    const star = starMatch ? starMatch[0] : null;

                    // This will pick only the percentage text like "78%", "13%", etc.
                    const percentText = $el.find('.a-text-right').first().text().trim().split('%');
                    const percent = percentText[index];
                    if (star !== null && percent !== null) {
                        this.reviewMetadata.stars_stat[star] = percent + '%';
                    }
                });
            } catch (e) {
                console.error('Error parsing review stats:', e);
                return;
            }
        }
        ...
        const reviewContainers = ['#localTopReviewsList', '#internationalTopReviewsList'];

        const scrapingResult = {};

        for (const container of reviewContainers) {
            $(`${container} div[data-hook="review"]`).each((_, review) => {
                const $review = $(review);

                const reviewId = $review.attr('id');
                if (!reviewId) return;

                if (this.bulk && Object.keys(scrapingResult).length + this.collector.length >= this.number) {
                    return false;
                }
                ...
                // Reviewer name
                reviewObj.name = $review.find('.a-profile-name').first().text().trim();

                // Review date
                reviewObj.review_data = $review.find('[data-hook="review-date"]').text().trim();
                if (reviewObj.review_data) {
                    reviewObj.date = this.geo.review_date(reviewObj.review_data);
                }

                // Rating
                const rating = $review.find('[data-hook="review-star-rating"] .a-icon-alt').text().trim();

                reviewObj.rating = parseFloat(rating);

                // Title
                reviewObj.title = $review
                    .find('[data-hook="review-title"], [data-hook="reviewTitle"]')
                    .text()
                    .replace(/^.*?out of 5 stars\s*/i, '')
                    .trim();

                // Review text
                reviewObj.review = $review
                    .find('[data-hook="reviewRichContentContainer"], [data-hook="review-collapsed"], [data-hook="review-body"]')
                    .text()
                    .trim();

                // Verified purchase
                reviewObj.verified_purchase = $review.find('[data-hook="avp-badge"], [data-reftag="cm_cr_arp_d_rvw_rvwer"]').length > 0;

                // Variant ASIN
                const href = $review.find('[data-hook="format-strip"], .review-format-strip a').attr('href');

                if (href) {
                    const match = href.match(/product-reviews\/([A-Z0-9]{10})/i) || href.match(/customer-reviews\/([A-Z0-9]{10})/i);

                    if (match) {
                        reviewObj.asin.variant = match[1];
                    }
                }

                // Images
                $review.find('img').each((_, img) => {
                    ...
                    src = src.replace(/_SX\d+_/g, '_SL1600_').replace(/_SY\d+_/g, '_SL1600_');

                    if (!reviewObj.media.includes(src)) {
                        reviewObj.media.push(src);
                    }
                });

                scrapingResult[reviewId] = reviewObj;
            });
        }

        this.collector.push(...Object.values(scrapingResult));
    }
    ...
    grabAsinDetails(body) {
        ...
        output.aplus_content = this.extractProductAplusContent($, body);
    ...
    /**
     * Extract A+ content and Brand Story content from a product page
     * Includes brand story images/text, A+ module images/text, comparison tables, and videos
     * @param {*} $ - cheerio instance
     * @param {*} body - raw HTML string
     */
    extractProductAplusContent($, body) {
        const aplusData = {
            brand_story: {
                images: [],
                text_blocks: [],
        ...
        /**
         * Helper: check if an image src is a real content image
         * (filters out spacers, sprites, pixel trackers, etc.)
         */
        const isValidAplusImage = (src) => {
            if (!src) return false;
            ...
            return (
                ...
                !lower.includes('1x1') &&
                !lower.startsWith('data:image/gif;base64,r0lgoddlh')
            );
        };

        /**
         * Helper: normalise image src to the highest available resolution
         */
        const normaliseImageSrc = (src) =>
            src
                .replace(/_SS\d+_/, '_SL1500_')
                .replace(/_SL\d+_/, '_SL1500_')
                .replace(/\._CR\d+,\d+,\d+,\d+_PT\d+_SX\d+_\.jpg$/, '.jpg');

        /**
         * Helper: strip invisible Unicode direction characters and collapse whitespace
         */
        ...
        /**
         * Extract images and text blocks from a given CSS selector into a target object
         */
        const processContainer = (selector, targetObj) => {
            ...
            container.find('img').each((_, imgEl) => {
                ...
                // Prefer data-src when it carries the high-res variant
                if (dataSrc && !src.includes('SL1500') && dataSrc.includes('SL1500')) {
                    src = dataSrc;
                }

                if (isValidAplusImage(src)) {
                    src = normaliseImageSrc(src);
                    const alt = cleanText($img.attr('alt') || '');

                    if (!targetObj.images.some((img) => img.src === src)) {
                        targetObj.images.push({ src, alt });
                    }
                    if (!aplusData.all_images.some((img) => img.src === src)) {
                        aplusData.all_images.push({ src, alt });
                    }
                }
            });
            ...
            // Text blocks
            container
                .find('h1, h2, h3, h4, h5, p, li, td, span.a-text-bold, span.a-size-base, .aplus-p1, .aplus-p2, .aplus-p3')
                .each((_, textEl) => {
                    const $text = $(textEl);
                    // Skip wrapper nodes whose children already produce their own entries
                    if ($text.find('h1, h2, h3, h4, h5, p').length > 0) return;
                    const txt = cleanText($text.text());
                    if (txt && txt.length > 2 && !targetObj.text_blocks.includes(txt)) {
                        targetObj.text_blocks.push(txt);
                    }
                });
        };
        ...
        // --- Brand Story ---
        try {
            processContainer('#brandStory_feature_div, #brandStory', aplusData.brand_story);
        } catch {
            // continue regardless of error
        }
        ...
        // --- A+ Content modules ---
        try {
            processContainer('#aplus_feature_div, #aplus, .aplus-v2, .premium-aplus', aplusData.aplus_content);
        } catch {
            // continue regardless of error
        }

        // --- Comparison tables inside A+ section ---
        try {
            const aplusContainer = $('#aplus_feature_div, #aplus, .aplus-v2, .premium-aplus');
            if (aplusContainer.length) {
                aplusContainer.find('table.comparison-table, table.a-bordered').each((_, tableEl) => {
                    const $table = $(tableEl);
                    const tableData = [];
                    ...
                    $table.find('tr').each((_, trEl) => {
                        ...
                        $(trEl)
                            ...
                            .each((_, tdEl) => {
                                ...
                                const cellImg = $cell.find('img').first();
                                let cellImgSrc = cellImg.length
                                    ? cellImg.attr('src') || cellImg.attr('data-src') || ''
                                    : '';
                                const cellImgAlt = cellImg.length ? cleanText(cellImg.attr('alt') || '') : '';

                                if (cellImgSrc && isValidAplusImage(cellImgSrc)) {
                                    cellImgSrc = normaliseImageSrc(cellImgSrc);
                                }

                                row.push({
                                    text: cellText,
                                    image: cellImgSrc ? { src: cellImgSrc, alt: cellImgAlt } : null,
                                });
                            });
                    ...
                    if (tableData.length > 0) {
                        aplusData.aplus_content.comparison_tables.push(tableData);
                    }
                });
            }
        } catch {
        ...
        // --- Fallback: combined container when sections are not split ---
        try {
            if (aplusData.all_images.length === 0 && aplusData.aplus_content.text_blocks.length === 0) {
                processContainer('#aplusAndBrandStoryContainer', aplusData.aplus_content);
            }
        } catch {
            // continue regardless of error
        }

        // --- Videos embedded in the A+/Brand Story sections ---
        try {
            $('[class*="vseVideoDataItem"], [data-video-url]').each((_, el) => {
                const $el = $(el);
                const videoAsin = $el.attr('data-asin') || '';
                const title = $el.attr('data-title') || '';
                const videoUrl = $el.attr('data-video-url') || '';

                if (videoAsin || title || videoUrl) {
                    const videoItem = {
                        asin: videoAsin,
                        title,
                        video_url: videoUrl,
                        thumbnail_url:
                            $el.attr('data-video-image-url-unchanged') || $el.attr('data-video-image-url') || '',
                        duration: $el.attr('data-formatted-duration') || '',
                        creator: {
                            name: $el.attr('data-public-name') || $el.attr('data-vendor-name') || '',
                            type: $el.attr('data-creator-type') || '',
                        },
                    };
                    if (!aplusData.videos.some((v) => v.video_url === videoItem.video_url && videoItem.video_url)) {
                        aplusData.videos.push(videoItem);
                    }
                }
            });
        } catch {
