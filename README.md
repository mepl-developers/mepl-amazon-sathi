...
```sh
$ npm i -g mepl-amazon-sathi
```

**Install from YARN**

```sh
$ yarn global add mepl-amazon-sathi
```
...
```sh
$ mepl-amazon-sathi --help

Usage: mepl-amazon-sathi <command> [options]

Commands:
  mepl-amazon-sathi products      collect products by using keyword
  mepl-amazon-sathi reviews [id]  collect reviews from product by using ASIN id
  mepl-amazon-sathi asin [id]     single product details
  mepl-amazon-sathi categories    get list of categories
  mepl-amazon-sathi countries     get list of countries
...
Options:
  ...
  --country       In ISO 3166 (Alpha-2 code) format. To get available list of
                  ...
                  mepl-amazon-sathi countries                [string] [default: "US"]
  --category      To get available list of categories type and use (index) from
                  the shown table as value: mepl-amazon-sathi categories
                                                       [string] [default: "aps"]
...
Examples:
  mepl-amazon-sathi products -k 'Xbox one'
  mepl-amazon-sathi products -k 'Xbox one' --country 'GB'
  mepl-amazon-sathi reviews B01GW3H3U8
  mepl-amazon-sathi asin B01GW3H3U8
  mepl-amazon-sathi categories
  mepl-amazon-sathi countries
```
...
```sh
$ mepl-amazon-sathi products -k 'vacuum cleaner' -n 40 --filetype csv
```
...
```sh
$ mepl-amazon-sathi products -k 'vacuum cleaner' -n 40 --filetype ''
```
...
```sh
$ mepl-amazon-sathi products -k 'vacuum cleaner' -n 40 --filetype '' --country NL
```
...
```sh
$ mepl-amazon-sathi products -k 'games' -n 40 --filetype '' --country ES --category mobile-apps
```
...
```sh
$ mepl-amazon-sathi reviews B01GW3H3U8 -n 100
```
...
```sh
$ mepl-amazon-sathi products -k 'xbox one' -n 300 --min-rating 3 --max-rating 4
```
...
```sh
$ mepl-amazon-sathi categories --country GB
```
...
```javascript
const amazonScraper = require('mepl-amazon-sathi');
...
(async () => {
    try {
        ...
        const reviews = await amazonScraper.products({
            keyword: 'Xbox One',
            bulk: false,
            page: 2,
        });
